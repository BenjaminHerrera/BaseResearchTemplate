...
class ExampleEvaluation(BaseEvaluation):
    """Example regression evaluation class that hosts the following metrics:
    ...
    def __init__(self: "ExampleEvaluation"):
        """Constructor to initialize a basic regression evaluation instance

        Args:
            self (ExampleEvaluation): Instance method for the class
        """
        # Inherit the attributes and methods of the parent class
        super(ExampleEvaluation).__init__()

    def __call__(
        self: "ExampleEvaluation",
        predictions: list,
        labels: list,
        inputs: torch.Tensor,
    ) -> dict[str, float]:
