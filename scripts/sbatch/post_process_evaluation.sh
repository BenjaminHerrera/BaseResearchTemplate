...
read -p "Where should the slurm output be (default is /scratch/%u/projects/<PROJECT_NAME>/slurm_dump/post_process_%j/): " parition_type
slurm_output=${slurm_output:-/scratch/%u/projects/<PROJECT_NAME>/slurm_dump/post_process_%j/}
...
#!BATCH --mail-user=<YOUR_EMAIL>                                                            # notify email (%u expands -> username)
#SBATCH --job-name="<PROJECT_NAME> // Post Process Evaluation - $execution_config"          # job name
